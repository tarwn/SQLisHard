...
{
	[TestFixture]
	public class EmailErrorReporterTests
	{
		private EmailErrorReporter CreateReporterMock() {
			var mockEmailFactory = new Mock<IFluentEmailFactory>();
			var mockSettings = new Mock<IOptions<EmailErrorSettings>>();
			mockSettings.Setup(x => x.Value).Returns(new EmailErrorSettings { toAddress = "test@example.com" });
			var mockEnvironment = new Mock<IWebHostEnvironment>();
			mockEnvironment.Setup(x => x.EnvironmentName).Returns("Test");
			var mockLogger = new Mock<ILogger<EmailErrorReporter>>();

			return new EmailErrorReporter(
				mockEmailFactory.Object,
				mockSettings.Object,
		...
		{
			var provider = CreateReporterMock();

			var result = provider.GetHeaderHtml(new Dictionary<string, string>());
		...
		{
			var provider = CreateReporterMock();
			var result = provider.GetHeaderHtml(new Dictionary<string, string>() { { "KEY", "VALUE" } });
		...
		{
			var provider = CreateReporterMock();
			var result = provider.GetHeaderHtml(new Dictionary<string, string>() { { "KEY", "VALUE,VALUE2" } });
			Assert.That(result, Is.EqualTo("<tr><th align='right' valign='top'>KEY:</th><td>VALUE,VALUE2</td></tr>"));
		}
		...
		public void BuildMessage_BasicException_IncludesStackTrace()
		{
			...
			Assert.That(message.Contains("EmailErrorReporterTests.cs"), Is.True);
		}
