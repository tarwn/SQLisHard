	...
	public class TemporaryExerciseStore : IExerciseStore
	{
		...
		private static DefinedExerciseSet _hardcodedExercises = new DefinedExerciseSet() {
			...
			Exercises = new List<DefinedExercise>(){
				...
				new DefinedExercise(){ Id = "S4.0", Title = "S4.0 - INNER JOIN", Details = "<p>Now that we can pull data out of a single table, let's take it a step further. JOIN statements allow us to 'join' the records of several tables together with instructions on how to match up the records in each</p><code>SELECT [columns] FROM <i>table_name</i> INNER JOIN <i>table_name2</i> ON <i>condition</i></code><p>Try using the INNER JOIN syntax to SELECT all columns from the Customers and Orders tables where the CustomerId column in Orders matches the Id column in Customers.</p>", Query = "SELECT * FROM Customers C INNER JOIN Orders O ON C.id = O.CustomerId" }
			},
